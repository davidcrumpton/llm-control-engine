...
async function main() {
  ...
  if (options.provider === 'lmstudio') {
    llm = new LMStudioProvider({ host: process.env.LLMCTRLX_API_URL || options.host, apiKey: options.__api_key })
  } else {
    llm = new OllamaProvider({ host: process.env.LLMCTRLX_API_URL || options.host, apiKey: options.__api_key })
  }
