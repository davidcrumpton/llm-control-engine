...
export async function cmdModel(llm: LLMProvider, options: CLIOptions) {
  ...
  if (actionKey) {
    ...
    if (!options.model) {
      console.error(
        `Error: Model name must be provided for ${action.label} command.`,
      );
      return;
    }
    ...
    if (!llm.capabilities?.includes(action.method)) {
      console.error(
        `${action.label.charAt(0).toUpperCase() + action.label.slice(1)} command is not supported by this provider.`,
      );
      return;
    }
    ...
    try {
      ...
      console.error(`Error executing ${action.label}: ${message}`);
    }
    return;
  }
  ...
  console.log(
    llm.getHelpMessage
      ? llm.getHelpMessage()
      : "model commands: --list, --show <model>, --pull <model>, --delete <model>",
  );
}
