...
function playback(session: Session) {
  console.error(
    `[replay] playback — ${session.command_type} recorded at ${session.recordedAt}`,
  );
  console.error(`[replay] runHash: ${session.runHash}`);

  if (session.outputs.error) {
    console.error(
      `[replay] ⚠ this session recorded an error: ${session.outputs.error}`,
    );
  }
...
async function reExecuteRun(
  ...
  if (Symbol.asyncIterator in res) {
    throw new Error("Streaming not supported in reExecuteRun");
  }
...
async function reExecuteChat(
  ...
  (chatOptions as any).onToolCall = makeToolCallRecorder(recorder);
  ...
  if (inputs.no_tools) {
    llmResponse = await runWithoutTools(
      llm,
      inputs.model,
      messages,
      chatOptions,
    ...
    if (requestedTags) {
      tools = tools.filter(
        (t) =>
          ...
          requestedTags.some((tag: string) => (t.tags || []).includes(tag)),
      );
    }
...
async function reExecutePlan(
  ...
  if (!inputs.planFile) {
    throw new Error(
      "Session inputs.planFile is missing — cannot re-execute plan.",
    );
  }
  ...
  if (inputs.vars) {
    syntheticOptions.vars = Object.entries(inputs.vars).map(
      ([k, v]) => `${k}=${v}`,
    );
  }
