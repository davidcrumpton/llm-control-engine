...
export function diffToolCalls(
  ...
  const frsh = fresh.filter((e): e is ToolCallEvent => e.type === "tool_call");
  const lines: string[] = [];
  let match = true;
...
export function diffSteps(
  ...
  const rec = recorded.filter((e): e is StepEvent => e.type === "step");
  const frsh = fresh.filter((e): e is StepEvent => e.type === "step");
  const lines: string[] = [];
  let match = true;
...
export function buildDiffReport(
  ...
  sections.push("\n══════════════════════════════════════════");
  sections.push(
    allMatch ? " VERDICT: ✓ REPRODUCIBLE" : " VERDICT: ✗ DIFFERENCES FOUND",
  );
  sections.push("══════════════════════════════════════════");
