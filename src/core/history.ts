...
export function loadHistory(file: string): HistoryRecord {
  ...
  try {
    ...
    console.warn(`WARN: Cannot read history file '${file}': ${message}`);
    return {};
  }
 ...
 */
export function saveHistory(file: string, data: HistoryRecord): void {
  try {
...
export function getHistoryWindow(
  ...
  if (!session.messages || session.messages.length === 0 || limit === -1)
    return [];
