...
describe("AnthropicProvider", () => {
  ...
  describe("complete", () => {
    it("should return a formatted message response", async () => {
      ...
      expect(ANTHROPIC_MESSAGE_RESPONSE.content[0].text).toBe(
    ...
    it("should handle tool_use content blocks", async () => {
      ...
      expect(ANTHROPIC_TOOL_USE_RESPONSE.content[0].name).toBe("get_weather");
    });

    it("should map Anthropic stop_reason to standard finishReason", async () => {
      mockCreate.mockResolvedValue(ANTHROPIC_MESSAGE_RESPONSE);
